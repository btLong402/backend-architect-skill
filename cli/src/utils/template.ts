...
export async function renderSkillFile(config: PlatformConfig): Promise<string> {
    ...
    // Replace placeholders in quick reference first
    let processedQuickRef = '';
    if (quickReferenceContent) {
    ...
    content = content
        ...
        .replace(/\{\{QUICK_REFERENCE\}\}/g, processedQuickRef);
...
async function ensureSharedExists(targetDir: string): Promise<boolean> {
    const sharedDir = join(targetDir, '.shared', 'backend-architect');
